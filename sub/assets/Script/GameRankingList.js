cc.Class({
        ...
        if (window.wx != undefined) {
            window.wx.onMessage(data => {
                ...
                if (data.messageType == 0) {//移除排行榜
                    ...
                    this.fetchFriendData(data.NUM);
                } else if (data.messageType == 3) {//提交得分
                    this.submitScore(data.NUM, data.score);
                } else if (data.messageType == 4) {//获取好友排行榜横向排列展示模式
                    this.gameOverRank(data.NUM);
                } else if (data.messageType == 5) {//获取群排行榜
                    this.fetchGroupFriendData(data.NUM, data.shareTicket);
                }
            });
        } else {
            ...
            window.wx.getUserCloudStorage({
                ...
                success: function (getres) {
                    ...
                    window.wx.setUserCloudStorage({
                        KVDataList: [{key: NUM, value: "" + score}],
    ...
    fetchGroupFriendData(NUM, shareTicket) {
        this.removeChild();
        this.rankingScrollView.node.active = true;
        if (window.wx != undefined) {
            wx.getUserInfo({
